...
## Tool calling（顺应身份的防御纵深）

PromptQL 的 agent 有很强的内置 system prompt 与 data/query assistant 身份，会**拒绝**「按 `<tool_call>` 围栏输出工具调用」这类直白指令（实测回复 *"that's not how I operate"*），甚至自带 wiki/data/code 工具自行回答。复杂场景（长 system + 多工具 + 多轮，如 Claude Code / Cline 驱动）下单一手段易被识破，故本网关用**多层防御**叠加命中率——参考 `/data1/cursor2api` 的工程方法，但全部针对 PromptQL 身份重写、且**工具/客户端无关**：

1. **system 软化包装**（`app/system_sanitizer.py`）：客户端 system 不再用硬标签 `[system]` 直塞，而是移除计费/调试头垃圾行后，用柔和的背景框架承载——弱化「系统强制身份覆盖」色彩，降低 agent 身份对抗刺激。**不动一个字的实质指令**（身份声明、工具指令、强制措辞一律保留）。
2. **认知重构角度集**（`app/reframe_angles.py`）：顺应 agent 身份，在消息最前注入情景，让 agent「只是在生成一段**表示**工具调用的记录」（职责内）而非「执行工具」。默认 **B「测试夹具」**（probe 实测稳定命中者）；新增 **F「结构化动作记录」/ G「路由标注」**更贴 agent `actions_parsed` 本职，作重试轮换备选（`scripts/probe_reframe.py --angles all` 验证后可替换默认）。
3. **拒绝检测 + 自动重试**（`app/orchestrator.py:stream_with_retry` + `app/refusal.py`）：整轮 buffer 回复 → 命中身份识破/拒绝措辞 → 换角度重建 prompt 重试（`config.toml [promptql].tool_call_retries`，默认 3），把单次命中率累积成多次。与 `deps._RetryingClient`（账号级 503 换号）正交。

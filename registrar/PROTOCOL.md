...
- 成功响应（otp 有效）：`200` + **`Set-Cookie: hasura-lux=...`**（httpOnly；新账号登录态根凭据）。该 cookie **host-only 于 `auth.pro.ql.app`**——实测 `data.pro.ql.app`、`prompt.ql.app` 等兄弟/父子域请求均不带它，故浏览器 `document.cookie` 读不到；油猴脚本需 Tampermonkey **Beta** + `@match auth.pro.ql.app` 用 `GM_cookie.list({url})` 跨域读取，否则只能 DevTools 手动复制。注册机用 curl-cffi 从响应 set-cookie 提取全值。
